...
public class Tests
{
    ...
    public async Task OnelineAddressToLocation()
    {
        ...
        Assert.That(response.addressMatches.First().coordinates.x, Is.EqualTo(-84.553809193124d));
        Assert.That(response.addressMatches.First().coordinates.y, Is.EqualTo(42.73373146656d));
    }
    ...
    public async Task BulkMemory()
    {
        ...
        Assert.That(response.First().lat, Is.EqualTo((-84.553809193124d).ToString()));
        Assert.That(response.First().lon, Is.EqualTo(42.73373146656d.ToString()));
    }
